    ...
    macro_rules! declare_err {
        ($err:tt $(,)? $($doc:expr),+) => {
            ...
            pub const $err: super::Error =
                match super::Error::try_from_errno(-(::bindings::$err as i32)) {
                    Some(err) => err,
...
impl Error {
    ...
    /// ```
    pub fn from_errno(errno: ffi::c_int) -> Error {
    ...
    const fn try_from_errno(errno: ffi::c_int) -> Option<Error> {
    ...
    ///
    /// `errno` must be within error code range (i.e. `>= -MAX_ERRNO && < 0`).
    const unsafe fn from_errno_unchecked(errno: ffi::c_int) -> Error {
    ...
    pub fn to_errno(self) -> ffi::c_int {
        self.0.get()
    }
    ...
    pub fn to_ptr<T>(self) -> *mut T {
        // SAFETY: `self.0` is a valid error due to its invariant.
        unsafe { bindings::ERR_PTR(self.0.get() as ffi::c_long).cast() }
    }
...
///
/// [`?`]: https://doc.rust-lang.org/reference/expressions/operator-expr.html#the-question-mark-operator
pub fn to_result(err: ffi::c_int) -> Result {
    if err < 0 {
...
pub fn from_err_ptr<T>(ptr: *mut T) -> Result<*mut T> {
    ...
    let const_ptr: *const ffi::c_void = ptr.cast();
    ...
    if unsafe { bindings::IS_ERR(const_ptr) } {
        ...
        // negative value greater-or-equal to `-bindings::MAX_ERRNO`.
        return Err(unsafe { Error::from_errno_unchecked(err as ffi::c_int) });
    }
    Ok(ptr)
}
